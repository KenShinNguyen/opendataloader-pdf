...
export function registerCliOptions(program: Command): void {
  ...
  program.option('--replace-invalid-chars <value>', 'Replacement character for invalid/unrecognized characters (a glyph whose font has no Unicode mapping - unrecoverable without OCR). Default: "?", so a missing glyph does not read as a fabricated word boundary the way a space would.');
